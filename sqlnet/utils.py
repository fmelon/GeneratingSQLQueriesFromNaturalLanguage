...
def epoch_acc(model, batch_size, sql_data, table_data, pred_entry):
    ...
    while st < len(sql_data):
        ...
        print_gt_query = query_gt
        print_pred_query = pred_queries 
        print_question_seq = raw_q_seq
        print_col_seq = raw_col_seq
        one_acc_num += (ed-st-one_err)
        tot_acc_num += (ed-st-tot_err)

        st = ed
    return tot_acc_num / len(sql_data), one_acc_num / len(sql_data), print_question_seq, print_col_seq, print_gt_query, print_pred_query
...
def print_responses(in_data):

        ### Natural language questions, Column names, Ground truth queries, Predicted queries
        natural_text = in_data[0]
        column_names = in_data[1]
        gt_queries = in_data[2]
        pred_queries = in_data[3]

        num_responses = len(pred_queries)
		...
		print("")
                print("Natural Language Text : " + str(natural_text[i]))

                gt_query = "SELECT "
                if gt_queries[i]['agg'] != 0:
                        gt_query = gt_query + agg_ops[gt_queries[i]['agg']] + "(" + column_names[i][gt_queries[i]['sel']]  + ") WHERE "
                else:
                        gt_query = gt_query + column_names[i][gt_queries[i]['sel']] + " WHERE "
                
                gt_query = gt_query + column_names[i][gt_queries[i]['conds'][0][0]] + " " + cond_ops[gt_queries[i]['conds'][0][1]] + " " + gt_queries[i]['conds'][0][2]
                print("Ground truth query : " + str(gt_query))

                pred_query = "SELECT "
                if pred_queries[i]['agg'] != 0:
                        pred_query = pred_query + agg_ops[pred_queries[i]['agg']] + "(" + column_names[i][pred_queries[i]['sel']]  + ") WHERE "
                else:
                        pred_query = pred_query + column_names[i][pred_queries[i]['sel']] + " WHERE "
                
                pred_query = pred_query + column_names[i][pred_queries[i]['conds'][0][0]] + " " + cond_ops[pred_queries[i]['conds'][0][1]] + " " + pred_queries[i]['conds'][0][2]
                print("Predicted query : " + str(pred_query))

                if gt_query.lower() == pred_query.lower():
                        print("Query match found")
		else:
			print("Query match not found")
		print("")
